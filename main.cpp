...
struct train_struct {
  ...
  int problem_number;
  train_struct(class_problem *p, std::vector<double> *r, int n) 
    : cp(p), results(r), problem_number(n) {}; 
};

void thread_train (void * prob_ptr) {
  train_struct *tr_str = (train_struct *) prob_ptr;
  libsvm problem;
  std::vector<profile*> data_points = tr_str->cp->get_positive_training();
  ...
  data_points = tr_str->cp->get_negative_training();
  for (std::vector<profile*>::iterator it = data_points.begin();
      it != data_points.end(); ++it)
    problem.addTrainSeq((*it)->get_representing_vector(),NEG);
  data_points = tr_str->cp->get_positive_testing();
  for (std::vector<profile*>::iterator it = data_points.begin();
      it != data_points.end(); ++it)
    problem.addTestSeq((*it)->get_representing_vector(),POS);
  data_points = tr_str->cp->get_negative_testing();
  for (std::vector<profile*>::iterator it = data_points.begin();
      it != data_points.end(); ++it)
...
int main(int argc, char** argv) {
  ...
  for(unsigned int i = 0; i < 3; ++i) {
    pool.addThread(thread_train, 
        (void *) new train_struct(cp_vector[i], &results, i));
  }
  pool.wait();
  double avg_roc = 0;
